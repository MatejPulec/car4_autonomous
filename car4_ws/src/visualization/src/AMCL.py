    ...
    while not rospy.is_shutdown():
        if latest_position is not None and latest_angle is not None:
            ...
            tf_broadcaster.sendTransform(
                ...
                quaternion,
                rospy.Time.now(),
                "AMCL_for_visualization",  # Child frame
                "map"  # Parent frame
            )
        rate.sleep()
...
def main():
    # SETUP
    LIDAR_ANGLE = 240
    LIDAR_NUMBER_OF_POINTS = 200  # 682
    ...
    tf_listener.waitForTransform(
        "map", "AMCL", rospy.Time(0), rospy.Duration(10.0))
    (translation, quaternion) = tf_listener.lookupTransform(
        "map", "AMCL", rospy.Time(0))
    position = [int(translation[0]/resolution),
                int(translation[1]/resolution*-1)]
