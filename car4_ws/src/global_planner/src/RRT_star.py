...
orphan_coordinates = [[509, 498], [497, 520], [540, 497], [650, 535], [1682, 594], [2570, 624], [2635, 634], [2702, 615], [2722, 337], [
    2740, 217], [2955, 240], [3063, 290], [2941, 418], [2923, 637], [2817, 624], [313, 533], [306, 642], [108, 640], [78, 547]]
...
def smooth_out_path(path):
    ...
    for index, point in enumerate(path):
        if index != 0 and index != last_point_idx:
            coordinates = bresenham(
                path[index-1][0], path[index-1][1], path[index+1][0], path[index+1][1])
...
# fix if final_step_size os too small and N goes out of bounds
def straighten_out_path(path):
    ...
    while index_start < last_point_idx:
        coordinates = bresenham(
            path[index_start][0], path[index_start][1], path[index_end][0], path[index_end][1])
        if check_coordinates_collision(coordinates, map) == True:
            N = np.ceil(np.linalg.norm([path[index_start][0] - path[index_end]
                        [0], path[index_start][1] - path[index_end][1]]) / final_step_size)
            delta_0 = (path[index_end][0] - path[index_start][0]) / N
            delta_1 = (path[index_end][1] - path[index_start][1]) / N
            for n in range(int(N)):
                path[index_start +
                     n][0] = np.round(path[index_start][0] + n * delta_0)
                path[index_start +
                     n][1] = np.round(path[index_start][1] + n * delta_1)
...
def rrt_star(exploration_bias, extra_search_time, orphan_coordinates):
    ...
    for i in range(max_iterations):
        ...
        if new_node.x == final_node.x and new_node.y == final_node.y:
            exploration_bias = 0
        rewire_radius = np.minimum(
            step_size, (ball_radius_constant * (np.log(i+1)/i+1))**(1/dimension))
...
def check_for_orphans(new_node, tree, orphan_tree):
    for orphan_node in orphan_tree:
        if is_collision_free(orphan_node, new_node):
            ...
            tree, orphan_tree = check_for_orphans(
                orphan_node, tree, orphan_tree)
...
def create_trajectory():
    ...
    tree, path = rrt_star(exploration_bias, extra_search_time,
                          orphan_coordinates)  # Example function
    final_node = Node(goal[0], goal[1])  # Example final node from your planner
    available_nodes = get_available_nodes(
        tree, final_node, step_size)  # Example
    final_node.parent = choose_parent(
        tree, available_nodes, final_node)  # Example

    if final_node.parent is not None:
        tree.append(final_node)

    path = get_path(final_node)
    path = interpolate_path(path, final_step_size)
